...
- [codex-transcript-deepseek-fix](https://github.com/TOBYCAI/codex-transcript-deepseek-fix) - Fix Codex transcript corruption caused by switching config mid-session via cc switch (DeepSeek ↔ OpenAI): malformed message ids, overlong reasoning content, rs_resp_ 404, bare-UUID item ids; ships a validation script. Install: `python3 ~/.codex/skills/.system/skill-installer/scripts/install-skill-from-github.py --repo TOBYCAI/codex-transcript-deepseek-fix --path . --name codex-transcript-deepseek-fix`
