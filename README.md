...
- [configure-codex-remote-ssh](https://github.com/Anchras/terminalbox-tools/tree/main/plugins/codex-remote-ssh/skills/configure-codex-remote-ssh) - Connect the official Codex app to a paid Terminalbox pane over key-only SSH. Verifies the published host fingerprint, never reads private-key contents, preserves existing SSH configuration, and checks Codex in the remote login shell. Install: `npx skills add Anchras/terminalbox-tools --skill configure-codex-remote-ssh`
